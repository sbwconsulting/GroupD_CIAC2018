...
# colnames(dt_ciac_gr)[grep('1|first',colnames(dt_ciac_gr),ignore.case = T)]
...
dt_ciac_proj_gr <- dt_ciac_gr[,.(meas_ct=length(ClaimID),ea_AnnGross_kWh=sum(ExAnte_Annualized_kWh,na.rm=T),ea_AnnGross_kW=sum(ExAnte_Annualized_kW,na.rm=T),
                           ...
                           ep_1stYrGross_kWh=sum(EvalExPostFirstYearGrosskWh,na.rm=T),ep_1stYrGross_kW=sum(EvalExPostFirstYearGrosskW,na.rm=T),
                           ep_1stYrGross_thm=sum(EvalExPostFirstYearGrossTherm,na.rm=T)),
...
dom_AnnGross_smry_el <- strat.ratio.estimator(dt_ciac_proj,period='Annualized',frame='Electric')
dom_AnnGross_smry_gas <- strat.ratio.estimator(dt_ciac_proj,period='Annualized',frame='Gas')
...
dom_1stYrGross_smry_el <- strat.ratio.estimator(dt_ciac_proj,period='1stYear',frame='Electric')
dom_1stYrGross_smry_gas <- strat.ratio.estimator(dt_ciac_proj,period='1stYear',frame='Gas')
dom_1stYrGross_smry <- merge(dom_1stYrGross_smry_el,dom_1stYrGross_smry_gas,by=c('PA','domain'),all=T,suffixes = c('_kWh','_thm'))

cnames_RR <- colnames(dom_1stYrGross_smry)[grep('^dom_eval_',colnames(dom_1stYrGross_smry))]
dom_1stYrGross_smry <- dom_1stYrGross_smry[,c('domain',cnames_RR),with=F]
setnames(dom_1stYrGross_smry,colnames(dom_1stYrGross_smry),c('domain',paste0(cnames_RR,'_1G')))
...
dom_Gross_smry <- merge(dom_Gross_smry,dom_1stYrGross_smry,by='domain')
...
if (bln_write) write.csv(dom_Gross_smry[,keep_cols_dom,with=F],fname_export_domGr,row.names = F)
...
if (bln_write) write.csv(dt_ciac_proj_gr_out,fname_export_projGross,row.names = F)

## PA Roll up----
PA_AnnGross_smry_el <- strat.ratio.estimator(dt_ciac_proj,frame='Electric',period='Annualized',return_PA = T)
PA_AnnGross_smry_gas <- strat.ratio.estimator(dt_ciac_proj,frame='Gas',period='Annualized',return_PA = T)
PA_AnnGross_smry <- merge(PA_AnnGross_smry_el,PA_AnnGross_smry_gas,all=T,suffixes=c('_kWh','_thm'),
                         by=c('PA'))
PA_LCGross_smry_el <- strat.ratio.estimator(dt_ciac_proj,frame='Electric',period='LifeCycle',return_PA = T)
PA_LCGross_smry_gas <- strat.ratio.estimator(dt_ciac_proj,frame='Gas',period='LifeCycle',return_PA = T)
PA_LCGross_smry <- merge(PA_LCGross_smry_el,PA_LCGross_smry_gas,all=T,suffixes=c('_kWh','_thm'),
                         by=c('PA'))
PA_1stYrGross_smry_el <- strat.ratio.estimator(dt_ciac_proj,period='1stYear',frame='Electric',return_PA = T)
PA_1stYrGross_smry_gas <- strat.ratio.estimator(dt_ciac_proj,period='1stYear',frame='Gas',return_PA = T)
PA_1stYrGross_smry <- merge(PA_1stYrGross_smry_el,PA_1stYrGross_smry_gas,by=c('PA'),all=T,suffixes = c('_kWh','_thm'))
cnames_eval_PA <- colnames(PA_1stYrGross_smry)[grep('^PA_eval_',colnames(PA_1stYrGross_smry))]
PA_1stYrGross_smry <- PA_1stYrGross_smry[,c('PA',cnames_eval_PA),with=F]
setnames(PA_1stYrGross_smry,colnames(PA_1stYrGross_smry),c('PA',paste0(cnames_eval_PA,'_1G')))
...
PA_Gross_smry <- merge(PA_Gross_smry,PA_1stYrGross_smry,by='PA')
...
grr_cols <- colnames(PA_Gross_smry)[grep('PA_.*gross',colnames(PA_Gross_smry))]
...
PA_Gross_smry[,c('PA',colnames(PA_Gross_smry)[grep('RR|rp',colnames(PA_Gross_smry))]),with=F]
PA_Gross_smry[,c('PA',colnames(PA_Gross_smry)[grep('RR.*LG',colnames(PA_Gross_smry))]),with=F]
if (bln_write) write.csv(PA_Gross_smry,fname_export_PA,row.names = F)

## SW Roll up----
## Annualized
SW_AnnGross_smry_el <- strat.ratio.estimator(dt_ciac_proj,frame='Electric',period='Annualized',return_SW = T)
...
SW_AnnGross_smry_gas <- strat.ratio.estimator(dt_ciac_proj,frame='Gas',period='Annualized',return_SW = T)
...
## Lifecycle
SW_LCGross_smry_el <- strat.ratio.estimator(dt_ciac_proj,frame='Electric',period='LifeCycle',return_SW = T)
...
SW_LCGross_smry_gas <- strat.ratio.estimator(dt_ciac_proj,frame='Gas',period='LifeCycle',return_SW = T)
...
## First (1st) Year
SW_1stYrGross_smry_el <- strat.ratio.estimator(dt_ciac_proj,frame='Electric',period='1stYear',return_SW = T)
gross_cols <- colnames(SW_1stYrGross_smry_el)[!grepl('kW',colnames(SW_1stYrGross_smry_el))]
new_gross_cols <- paste0(gross_cols,rep('_kWh',3))
setnames(SW_1stYrGross_smry_el,gross_cols,new_gross_cols)
SW_1stYrGross_smry_gas <- strat.ratio.estimator(dt_ciac_proj,frame='Gas',period='1stYear',return_SW = T)
gross_cols <- colnames(SW_1stYrGross_smry_gas)[!grepl('thm',colnames(SW_1stYrGross_smry_gas))]
new_gross_cols <- paste0(gross_cols,rep('_thm',3))
setnames(SW_1stYrGross_smry_gas,gross_cols,new_gross_cols)
SW_1stYrGross_smry <- cbind(SW_1stYrGross_smry_el,SW_1stYrGross_smry_gas)
FirstYr_cols <- setdiff(colnames(SW_1stYrGross_smry),c('domain','SW_pop_kWh','SW_n_kWh','SW_cmplt_kWh','SW_pop_thm','SW_n_thm','SW_cmplt_thm'))
setnames(SW_1stYrGross_smry,FirstYr_cols,paste0(FirstYr_cols,'_1G'))

## combine Annualized, Lifecycle, and First (1st) Year
SW_Gross_smry <- cbind(SW_AnnGross_smry,SW_LCGross_smry[,setdiff(colnames(SW_LCGross_smry),colnames(SW_AnnGross_smry)),with=F])
SW_Gross_smry <- cbind(SW_1stYrGross_smry,SW_Gross_smry[,setdiff(colnames(SW_Gross_smry),colnames(SW_1stYrGross_smry)),with=F])
...
if (bln_write) write.csv(SW_Gross_smry,fname_export_SW,row.names = F)
